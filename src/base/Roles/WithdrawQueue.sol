...
contract WithdrawQueue is Auth, ReentrancyGuard, IPausable {
    ...
    error WithdrawQueue__CallerNotOriusVault();
    error WithdrawQueue__CannotWithdrawOriusToken();
    ...
    function withdrawNonOriusToken(ERC20 token, uint256 amount) external {
        if (msg.sender != address(oriusVault)) revert WithdrawQueue__CallerNotOriusVault();
        if (address(token) == address(oriusVault)) revert WithdrawQueue__CannotWithdrawOriusToken();
    ...
    function _completeWithdraw(
        ...
        if (pullFundsFromVault) {
            ...
            oriusVault.exit(address(this), asset, totalAssets, address(this), shares);
        } else {
            // Burn shares.
            oriusVault.exit(address(this), asset, 0, address(this), shares);
        }
