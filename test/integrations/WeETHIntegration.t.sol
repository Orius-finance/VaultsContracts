...
contract WeETHIntegrationTest is Test, MerkleTreeHelper {
    ...
    function setUp() external {
        ...
        rawDataDecoderAndSanitizer = address(new WeETHFullDecoderAndSanitizer(address(oriusVault)));
    ...
    function testWeETHIntegration() external {
        ...
        _addWeETHLeafs(
            ...
            getAddress(sourceChain, "oriusVault") //referral
        );
        ...
        //sanity check
        uint256 weETHBalance = getERC20(sourceChain, "WEETH").balanceOf(address(oriusVault));
        assertGt(weETHBalance, 0);
    }

    function testWeETHIntegration__Reverts() external {
        deal(address(oriusVault), 1);
