...
contract TellerWithRemediationTest is Test, MerkleTreeHelper {
    ...
    function setUp() external {
        ...
        teller = new TellerWithRemediation(address(this), address(oriusVault), address(accountant), address(WETH));
    ...
    function testRemediation() external {
        ...
        deal(address(oriusVault), victim0, amountOfSharesStolen, true);

        // Assume phisher is able to steal victim0s shares.
        vm.prank(victim0);
        oriusVault.transfer(phisher, amountOfSharesStolen);
        ...
        oriusVault.transfer(vm.addr(1), amountOfSharesStolen);
        vm.stopPrank();
        ...
        assertEq(oriusVault.balanceOf(phisher), 0, "Phisher should have had shares removed.");
        assertEq(oriusVault.balanceOf(victim0), amountOfSharesStolen, "Victim0 should have had shares returned.");
        ...
        deal(address(oriusVault), victim1, amountOfSharesStolen, true);
        ...
        assertEq(oriusVault.balanceOf(phisher), 0, "Phisher should have had shares removed.");
        assertEq(
            oriusVault.balanceOf(remediator),
    ...
    function testCancellingRemediation() external {
        ...
        address user = vm.addr(1);
        deal(address(oriusVault), user, 1e18, true);
        ...
        oriusVault.transfer(address(this), 1e18);
        vm.stopPrank();
